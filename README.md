# 👩🏼‍💻 Giovanna Bonifacio
**`iOS Developer`** 

Em 2023, entrei no projeto Apple Developer Academy | Mackenzie, onde desenvolvi tanto habilidades sociais quanto técnicas. Participei de diversos projetos, que permitiram aplicar minha criatividade na resolução de problemas e encontrar soluções inovadoras. Tenho grande interesse em desenvolvimento iOS, e já conto com 4 aplicativos postados na AppStore. Todos os projetos foram desenvolvidos desde a ideação para resolver um problema real, até a programação utilizando Swift, SwiftUI e UIKit. 

<div style="display: inline_block"><br>
  <img align="center" alt="Swift" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/swift/swift-original.svg">
  <img align="center" alt="HTML" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg">
  <img align="center" alt="CSS" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg">
  <img align="center" alt="Python" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg">
  <img align="center" alt="JavaScript" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg">
</div>
  ##
<div>
  <a href = "mailto:bonifaciocgiovanna@gmail.com"><img src="https://img.shields.io/badge/-Gmail-%23333?style=for-the-badge&logo=gmail&logoColor=white" target="_blank"></a>
  <a href="www.linkedin.com/in/giovanna-bonifacio" target="_blank"><img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" target="_blank"></a>
</div>


## Simplin
🧩 O aplicativo **Simplin** foi desenvolvido para auxiliar a rotina de pessoas com **Transtorno do Espectro Autista (TEA)** por meio de uma agenda visual.  <br><br>
📕 A plataforma permite que usuários organizem suas atividades diárias de forma clara, utilizando **auxílios visuais** que contribuem para uma rotina mais previsível e menos estressante.   <br><br>
🦻🏼 O projeto priorizou a **acessibilidade**, implementando funcionalidades como **Dynamic Type** e **VoiceOver** da Apple, garantindo que o app seja acessível para varios usuários.  <br><br>
👩🏼‍💻 Foi desenvolvido com **SwiftUI**, com uso de **LocalNotification** para lembretes e **CoreData** para armazenamento seguro dos dados dos usuários.  <br><br>
📃 Em colaboração com colegas da **Apple Developer Academy | Mackenzie**, coautorei um artigo científico sobre o aplicativo que foca na importância da acessibilidade em soluções tecnológicas, que foi aprovado para o **congresso LACLO 2024**. <br> <br>
[Link na App Store: Simplin - Organize sua rotina](https://apps.apple.com/br/app/simplin-organize-sua-rotina/id6469009877) <br> 


<div style="display: flex; justify-content: center;">
  <img src="https://github.com/user-attachments/assets/8ed4c335-b204-441c-8ffa-114615361c6e" alt="VISUALIZACAO_TAREFA" width="200" style="margin-right: 500px;">
  <img src="https://github.com/user-attachments/assets/35b93f30-36d0-42ba-9c45-2c5632c389b1" alt="CRIACAO_TAREFA" width="200">
</div>



## Tentálculos 

🕹️ O aplicativo é um Serious Game desenvolvido para ajudar alunos do **Ensino Fundamental 2** a aprenderem **equações de 1° grau** de maneira divertida e interativa. <br><br>
🧮 O jogo utiliza o conceito visual de uma **balança**, tornando o aprendizado mais intuitivo e ajudando os estudantes a entenderem o equilíbrio em equações. <br><br>
👩🏼‍💻 O projeto foi desenvolvido utilizando o framework SpriteKit e SwiftUI.  <br><br>
🐙 O Tentalculos foi projetado para promover o aprendizado prático e acessível, utilizando a gamificação.  <br><br>
🗣️ Durante o desenvolvimento do jogo, além de desenvolvedora, eu fui a pessoa responsável pelo produto. Nessa função, eu liderei a equipe, planejei as entregas e priorizei as funcionalidades. <br> <br>
[Link na App Store: Tentalculos - Aprendendo Equações](https://apps.apple.com/br/app/tent%C3%A1lculos/id6477624788) <br>

<img src="https://github.com/user-attachments/assets/4af8c932-0b3e-4ba9-991d-9855bca6b4a3" alt="EQUACAO" width="200" style="margin-left: 20px;">
<img src="https://github.com/user-attachments/assets/d89d1b82-1e30-41ba-b8ec-5723eefacf60" alt="RESOLUCAO" width="200" style="margin-right: 20px;">

## Tá na mesa!

🍽️ O aplicativo é voltado para planejamento de receitas para a semana, gerando uma lista de compras. <br> <br>
👩🏼‍💻 Foi desenvolvido em SwiftUI, tanto para iPhone como iPad. <br> <br>
[Link na App Store: Ta na mesa!](https://apps.apple.com/br/app/t%C3%A1-na-mesa/id6483495910) <br>

<img src="https://github.com/user-attachments/assets/0c06eb22-c1c8-43af-ad06-b716a592472a" alt="RECEITA" width="200" style="margin-left: 20px;">
<img src="https://github.com/user-attachments/assets/1a3cce7b-a476-4738-b0f0-6a20b9e99a61" alt="LISTA_COMPRAS" width="200" style="margin-left: 20px;">
<img src="https://github.com/user-attachments/assets/e4f3d2c2-60a3-4334-8aba-c6d504197ed1" alt="PLANEJAMENTO" width="300" style="margin-left: 20px;">

## Pashion

👠 **Pashion** é um aplicativo desenvolvido para **estudantes e iniciantes de moda**, oferecendo ferramentas essenciais para a criação de peças e coleções.  <br> <br>
🎨 Pashion inclui funcionalidades de **desenho artístico** com ferramentas personalizadas, como pincéis ajustáveis em espessura e cor, além de uma **paleta de cores** variada na barra de ferramentas, ideal para representar diferentes estilos e materiais.  <br> <br>
👗 Os usuários podem gerenciar camadas de desenho, adicionar formas, imagens e texto, além de contar com uma ferramenta de espelho para criar designs simétricos.  <br> <br>
📂 O aplicativo permite também **organizar os documentos em coleções**, facilitando a visualização e o planejamento de peças dentro de um mesmo tema ou linha criativa.  <br> <br>
📄 **Fichas técnicas** podem ser criados no Pashion para documentar todos os detalhes técnicos das peças, que são salvos no app para consulta e compartilhamento.  <br> <br>
🖨️ O aplicativo também inclui recursos de exportação, permitindo que os usuários compartilhem suas e fichas técnicas com facilidade.  <br> <br>
👩🏼‍💻 Para as telas de desenho e moodboard utilizamos UIKit, e para as navegações e telas restantes foi utilizado SwiftUI. <br> <br>
[Link na App Store: Pashion - Creating Fashion from your Passion](https://apps.apple.com/br/app/pashion/id6737167821) <br>

<img src="https://github.com/user-attachments/assets/394ad9d4-8766-4b4c-a28c-31d0a665c459" alt="TELA_INICIAL" width="300" style="margin-left: 20px;">
<img src="https://github.com/user-attachments/assets/df99104b-d152-4903-ba15-0b17b4f83590" alt="DESENHO" width="300" style="margin-left: 20px;">
<img src="https://github.com/user-attachments/assets/50b2ec55-dcf6-4528-bf4e-0a93661bd9f9" alt="CRIACAO_FICHA" width="300" style="margin-left: 20px;">
<img src="https://github.com/user-attachments/assets/83ab88e8-8151-4924-8163-e6056ff050a8" alt="FICHA" width="300" style="margin-left: 20px;">

## Game of life 
 
Alem dos aplicativos que desenvolvi, fiz esse projeto com base nas regras do Game of Life, que explora a evolução de padrões a partir de regras simples. <br> <br>
No projeto, células são organizadas em uma grade bidimensional onde cada célula pode estar viva ou morta, evoluindo ao longo das gerações. <br> <br> 
As regras implementadas incluem: <br>
- Superpopulação: células vivas com mais de três vizinhos vivos morrem.
- Subpopulação: células vivas com menos de dois vizinhos vivos morrem.
- Continuação da vida: células vivas com dois ou três vizinhos vivos continuam vivas.
- Reprodução: células mortas com exatamente três vizinhos vivos tornam-se vivas. <br>


  



<img src="https://github.com/user-attachments/assets/1a48d597-baa9-403b-b761-51e18d629db9" alt="GAME_OF_LIFE" width="200">
<img src="https://github.com/user-attachments/assets/cbb7ffce-0191-41b2-a161-88e6d09370a4" alt="GAME_OF_LIFE" width="200">


<!-- Imagem 2 -->





<!-- Proudly created wi![1320x2868-2](https://github.com/user-attachments/assets/ab22d794-e97f-4ef6-b759-98ddb386e02d)
th GPRM ( https://gprm.itsvg.in ) -->



<!--
**gibonifacio/gibonifacio** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:
![IMG_1058](https://github.com/user-attachments/assets/98132126-acac-491b-98c1-e202eb5ae020)

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...![IMG_1057](https://github.com/user-attachments/assets/871ddb60-41e6-4345-a1ad-aee7826ac45d)

- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

